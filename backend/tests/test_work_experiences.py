...
def created_exp(client, auth_headers):
    ...
    res = client.post("/v1/work-experiences", json=BASE_PAYLOAD, headers=auth_headers)
...
def test_create_current_job(client, auth_headers):
    ...
    res = client.post("/v1/work-experiences", json=payload, headers=auth_headers)
    assert res.status_code == 201
...
def test_create_requires_auth(client):
    ...
    res = client.post("/v1/work-experiences", json=BASE_PAYLOAD)
...
def test_list_work_experiences(client, auth_headers, created_exp):
    ...
    res = client.get("/v1/work-experiences", headers=auth_headers)
...
def test_list_returns_only_own_experiences(client, auth_headers):
    ...
    client.post("/v1/work-experiences", json=BASE_PAYLOAD, headers=auth_headers)

    # Register and log in as user two
    client.post("/v1/auth/register", json={"email": "other2@example.com", "password": "pass"})
    login = client.post("/v1/auth/login", json={"email": "other2@example.com", "password": "pass"})
    other_headers = {"Authorization": f"Bearer {login.json()['access_token']}"}

    # User two's list should be empty
    res = client.get("/v1/work-experiences", headers=other_headers)
    assert res.json() == []


def test_list_ordered_by_order_field(client, auth_headers):
    """Entries should be returned sorted by the 'order' field ascending."""
    client.post("/v1/work-experiences", json={**BASE_PAYLOAD, "order": 2}, headers=auth_headers)
    client.post("/v1/work-experiences", json={**BASE_PAYLOAD, "order": 0}, headers=auth_headers)
    client.post("/v1/work-experiences", json={**BASE_PAYLOAD, "order": 1}, headers=auth_headers)
...
def test_delete_nonexistent_returns_404(client, auth_headers):
    ...
    res = client.delete("/v1/work-experiences/nonexistent-id", headers=auth_headers)
    assert res.status_code == 404
